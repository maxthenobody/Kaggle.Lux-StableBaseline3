...
class LuxAIS3GymEnv(gym.Env):
    def __init__(self, numpy_output: bool = False):
        ...
        self.observation_space = spaces.Dict({
            ...
            "steps": spaces.Box(low=0, high=505, shape=(), dtype=np.int32),  # Current step in match
            "match_steps": spaces.Box(low=0, high=100, shape=(), dtype=np.int32),  # Total match steps

            ### Random parameters
            ## Given
            # unit_move_cost=list(range(1, 6)), # list(range(x, y)) = [x, x+1, x+2, ... , y-1]
            "unit_move_cost": spaces.Box(low=1, high=5, shape=(), dtype=np.int32),
            # unit_sensor_range=list(range(2, 5)),
            "unit_sensor_range": spaces.Box(low=2, high=4, shape=(), dtype=np.int32),
            # unit_sap_cost=list(range(30, 51)),
            "unit_sap_cost": spaces.Box(low=30, high=50, shape=(), dtype=np.int32),
            # unit_sap_range=list(range(3, 8)),
            "unit_sap_range": spaces.Box(low=3, high=7, shape=(), dtype=np.int32),
            ## Not given
